...
door = next((m for m in snap0["markers"] if "-" in m["label"] and m["label"] not in ("entry",) and m["thr"] >= 0), None)
...
move(entry["ref"], entry["x"], entry["y"] + 150.0, entry["z"])  # try to slide the entry
dd_drag.sync()
snapB = snapshot()
...
check("3.peer did not move (no origin float)", bool(peer_still),
      f"{(peer['x'],peer['y'])}->{(peerB['x'],peerB['y'])}" if peerB else "peer gone")
...
snapC = snapshot()
victim = next((m for m in snapC["markers"] if m["label"] not in ("entry",) and m["label"] != door["label"]
